...
export function RefuelingImportModal() {
  ...
  const handleVehicleChange = async (vehicleId: string) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_SERVER_ORIGIN}/api/groups/getGroupByVehicle`, {
        method: 'POST',
  ...
  const handleUpload = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_SERVER_ORIGIN}/api/refuelings/upload`, {
        method: 'POST',
        body: formData,
      });
